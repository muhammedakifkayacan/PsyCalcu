          ...
          {activeTab === 'agenda' && (
            ...
            >
              ...
              <div className="lg:col-span-8 flex flex-col gap-6 order-1 lg:order-2" id="daily-agenda-section">
                ...
                {/* Günlük Rapor ve Klinik Değerlendirme Kartı */}
                {(() => {
                  const activeSessions = filteredSessions.filter(s => s.type !== 'cancelled' && s.type !== 'non-session');
                  const cancelledCount = filteredSessions.filter(s => s.type === 'cancelled').length;
                  const onlineCount = activeSessions.filter(s => s.type === 'online').length;
                  const faceToFaceCount = activeSessions.filter(s => s.type === 'face-to-face').length;
                  const unpaidSessions = activeSessions.filter(s => s.paymentStatus !== 'paid');
                  const unpaidSum = unpaidSessions.reduce((sum, s) => sum + (Number(s.price) || 0), 0);

                  const quotes = [
                    "\"Bir insanı dinlemek, ona var olma hakkı tanımaktır.\" - Carl Rogers. Seans sonrası kendinize de şefkat göstermeyi unutmayın.",
                    "Zihinsel ve duygusal emeğiniz çok değerli. Bugün dokunduğunuz yaşamlar ve gösterdiğiniz klinik özen için teşekkürler.",
                    "Klinik verimlilik sadece seans adediyle değil, seans kalitesi ve kendi ruhsal enerjinizi koruma becerinizle ölçülür.",
                    "Sınır çizmek ve dinlenmek bencilce bir davranış değildir; mesleki sürdürülebilirliğiniz için temel bir sorumluluktur.",
                    "Günün klinik yoğunluğu geride kaldı; şimdi zihninizi arındırma, kendinize şefkat gösterme ve dinlenme vakti.",
                    "Her seans yeni bir keşif yolculuğudur; mesleki sezgilerinize, bilginize ve terapötik becerilerinize güvenin.",
                    "\"Kendi içine bakmaya cesareti olanlar, başkalarının iç dünyasına rehberlik edebilirler.\" - C.G. Jung."
                  ];
                  const dayOffset = selectedDate ? parseInt(selectedDate.split('-').pop() || '0', 10) : 0;
                  const quoteIndex = (activeSessions.length + dayOffset) % quotes.length;
                  const selectedQuote = quotes[quoteIndex];

                  // Workload intensity tags & text
                  let workloadTag = "";
                  let workloadBg = "";
                  let workloadText = "";
                  let workloadDesc = "";
                  ...
                  if (activeSessions.length === 0) {
                    workloadTag = "Klinik Hazırlık & Dinlenme";
                    workloadBg = "bg-slate-100 text-slate-800 border-slate-200";
                    workloadText = "Sakin Gün";
                    workloadDesc = "Bugün aktif seansınız bulunmuyor. İdari işlerinizi düzenlemek, literatür okuması yapmak veya tamamen zihninizi dinlendirmek için mükemmel bir fırsat!";
                  } else if (activeSessions.length >= 1 && activeSessions.length <= 3) {
                    workloadTag = "Sürdürülebilir & Dengeli";
                    workloadBg = "bg-emerald-50 text-emerald-800 border-emerald-200";
                    workloadText = "Dengeli İş Yükü";
                    workloadDesc = `Bugün ${activeSessions.length} aktif seans gerçekleştirdiniz (${onlineCount} Online, ${faceToFaceCount} Yüz Yüze). Seans aralarınız dengeli ve tükenmişlik riskinden uzak, sürdürülebilir bir klinik akış sağlandı.`;
                  } else {
                    workloadTag = "Yüksek Klinik Yoğunluk";
                    workloadBg = "bg-amber-50 text-amber-800 border-amber-200/60";
                    workloadText = "Yoğun Gün";
                    workloadDesc = `Bugün ${activeSessions.length} aktif seans ile yüksek bir klinik yoğunluk yaşadınız. Seanslar arası zihinsel geçişlerinizde dinlenmeye ve kendinize özel bir nefes alanı ayırmaya özen gösterin.`;
                  }

                  return (
                    <div id="daily-report-card" className="bg-white rounded-[2rem] border border-[#e5e1d8] overflow-hidden shadow-xs mt-6 transition-all duration-300">
                      {/* Accordion Header */}
                      <button 
                        onClick={() => setShowAiDetails(!showAiDetails)}
                        className="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 p-5 md:p-6 hover:bg-[#faf9f6]/40 transition-colors text-left cursor-pointer"
                      >
                        <div className="flex items-center gap-3.5">
                          <div className="w-11 h-11 bg-[#fdfbf7] rounded-2xl flex items-center justify-center text-[#cb997e] shrink-0 border border-[#e5e1d8]/50 shadow-2xs">
                            <FileText className="w-5.5 h-5.5 text-[#cb997e]" />
                          </div>
                          <div>
                            <div className="flex items-center gap-2 flex-wrap">
                              <h4 className="text-sm font-bold text-slate-800 tracking-tight font-sans">Günlük Rapor & Değerlendirme</h4>
                              <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-bold border ${workloadBg}`}>
                                {workloadText}
                              </span>
                            </div>
                            <p className="text-xs text-slate-400 mt-0.5 font-medium">
                              {activeSessions.length > 0 
                                ? `${activeSessions.length} seans • ₺${dailySummary.net.toLocaleString('tr-TR')} Net Kâr ${unpaidSum > 0 ? `• ₺${unpaidSum.toLocaleString('tr-TR')} Bekleyen` : ''}`
                                : 'Bugün seansınız bulunmuyor • Dinlenme günü'
                              }
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2 self-stretch sm:self-auto justify-end">
                          <span className="px-4 py-2 bg-slate-50 hover:bg-slate-100 text-slate-600 rounded-full text-xs font-semibold flex items-center gap-1.5 transition-all border border-[#e5e1d8]">
                            {showAiDetails ? (
                              <>
                      ...
                      <AnimatePresence>
                        {showAiDetails && (
                          ...
                          >
                            <div className="p-6 md:p-8 bg-gradient-to-b from-white to-[#faf9f6]/30 space-y-6">
                              {/* Bento Grid layout */}
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                {/* Klinik İş Yükü Kutusu */}
                                <div className="bg-white rounded-2xl border border-[#e5e1d8]/80 p-5 space-y-3.5 shadow-2xs">
                                  <div className="flex items-center justify-between">
                                    <h5 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                                      <Clock className="w-4 h-4 text-[#6b705c]" />
                                      KLİNİK YOĞUNLUK ANALİZİ
                                    </h5>
                                    <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-bold border ${workloadBg}`}>
                                      {workloadTag}
                                    </span>
                                  ...
                                  {activeSessions.length > 0 && (
                                    <div className="grid grid-cols-3 gap-3 pt-1">
                                      <div className="bg-[#faf9f6] p-2.5 rounded-xl text-center border border-[#e5e1d8]/30">
                                        <span className="block text-[10px] text-slate-400 font-bold">ONLINE</span>
                                        <span className="text-sm font-bold font-serif italic text-slate-700">{onlineCount} Seans</span>
                                      </div>
                                      <div className="bg-[#faf9f6] p-2.5 rounded-xl text-center border border-[#e5e1d8]/30">
                                        <span className="block text-[10px] text-slate-400 font-bold">YÜZ YÜZE</span>
                                        <span className="text-sm font-bold font-serif italic text-slate-700">{faceToFaceCount} Seans</span>
                                      </div>
                                      <div className="bg-[#faf9f6] p-2.5 rounded-xl text-center border border-[#e5e1d8]/30">
                                        <span className="block text-[10px] text-slate-400 font-bold">İPTAL EDİLEN</span>
                                        <span className="text-sm font-bold font-serif italic text-slate-700">{cancelledCount} Seans</span>
                                      </div>
                                    </div>
                                  )}
                                </div>

                                {/* Finansal Performans Kutusu */}
                                <div className="bg-white rounded-2xl border border-[#e5e1d8]/80 p-5 space-y-3.5 shadow-2xs">
                                  <h5 className="text-xs font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                                    <TrendingUp className="w-4 h-4 text-emerald-600" />
                                    FİNANSAL PERFORMANS & TAHSİLAT
                                  </h5>
                                  <div className="space-y-2.5">
                                    <div className="flex justify-between items-center text-xs border-b border-slate-50 pb-2">
                                      <span className="text-slate-500 font-medium">Toplam Brüt Gelir:</span>
                                      <span className="font-bold text-slate-700">₺{dailySummary.gross.toLocaleString('tr-TR')}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-xs border-b border-slate-50 pb-2">
                                      <span className="text-slate-500 font-medium">Toplam Giderler (Kira + Bakıcı):</span>
                                      <span className="font-bold text-rose-600">₺{(dailySummary.babysitter + dailySummary.officeRent).toLocaleString('tr-TR')}</span>
                                    </div>
                                    <div className="flex justify-between items-center text-xs border-b border-slate-50 pb-2">
                                      <span className="text-slate-500 font-semibold">Net Dönem Kârı:</span>
                                      <span className="font-bold text-emerald-700">₺{dailySummary.net.toLocaleString('tr-TR')}</span>
                                    </div>
                                  </div>
                                  <div className="text-[11px] text-slate-500 bg-slate-50 p-2.5 rounded-xl border border-slate-100 flex items-start gap-1.5 leading-normal">
                                    <span className="text-sm">💡</span>
                                    <div>
                                      {unpaidSum > 0 ? (
                                        <span>
                                          Bugün tamamlanan seanslardan <strong className="text-[#cb997e]">{unpaidSessions.length} adedinin (₺{unpaidSum.toLocaleString('tr-TR')})</strong> ödemesi henüz tahsil edilmemiş. Akşam saatlerinde danışanlara nazik bir hatırlatma mesajı gönderebilirsiniz.
                                        </span>
                                      ) : activeSessions.length > 0 ? (
                                        <span>Bugün tamamlanan tüm seansların ödemeleri tahsil edildi! Finansal nakit akışınız kusursuz ilerliyor.</span>
                                      ) : (
                                        <span>Bugün herhangi bir seans olmadığı için aktif ödeme veya tahsilat beklentisi bulunmuyor.</span>
                                      )}
                                    </div>
                                  </div>
                                </div>
                              ...
                              {/* Terapist Motivasyon / Şefkat Kutusu */}
                              <div className="bg-[#fcfbf9] rounded-2xl border border-amber-100 p-5 space-y-2 text-center md:text-left shadow-2xs relative overflow-hidden">
                                <div className="absolute right-4 bottom-4 text-amber-200/20 pointer-events-none">
                                  <span className="text-7xl font-serif">"</span>
                                </div>
                                <div className="flex items-center gap-2 justify-center md:justify-start">
                                  <span className="text-sm">🌸</span>
                                  <h6 className="text-xs font-bold text-amber-900 tracking-wider">GÜNÜN KLİNİK MOTİVASYONU & ÖNERİSİ</h6>
                                </div>
                                <p className="text-xs text-slate-600 font-serif italic leading-relaxed max-w-4xl pt-0.5">
                                  {selectedQuote}
                                </p>
                              </div>
                            </div>
