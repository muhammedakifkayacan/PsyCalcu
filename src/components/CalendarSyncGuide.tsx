...
export default function CalendarSyncGuide({
  ...
  const handleSyncOnline = async () => {
    ...
    try {
      ...
      const parsed = parseICS(icsText, onlineDefaultPrice, defaultBabysitterFee, defaultOfficeRentFee, 'online', null, settings.autoMarkShortEventsAsNonSession ?? true);
  ...
  const handleSyncFaceToFace = async () => {
    ...
    try {
      ...
      const parsed = parseICS(icsText, faceToFaceDefaultPrice, defaultBabysitterFee, defaultOfficeRentFee, 'face-to-face', null, settings.autoMarkShortEventsAsNonSession ?? true);
      const isValidIcs = icsText.toUpperCase().includes('BEGIN:VCALENDAR') || icsText.toUpperCase().includes('BEGIN:VEVENT');
        ...
        <div className="relative bg-emerald-50/40 border border-emerald-100/80 p-6 rounded-2xl text-emerald-900 space-y-3 shadow-xs animate-fade-in pr-12">
          ...
          <div className="bg-white/95 p-4 rounded-xl border border-emerald-200/50 space-y-2 text-xs">
            ...
            <ul className="list-disc pl-5 space-y-1 text-slate-600">
              ...
              <li><strong className="font-semibold text-slate-800">Otomatik Kişisel Etkinlik Filtresi (#seansdeğil):</strong> Takviminizdeki spor, alışveriş, kuaför, doktor, diş randevusu, tatil, yemek veya toplantı gibi seans dışı planlar otomatik tespit edilir. Ayrıca herhangi bir etkinliğin adına veya açıklamasına <code className="bg-amber-100 text-amber-900 font-bold px-1.5 py-0.5 rounded text-[11px]">#seansdeğil</code> eklediğinizde öncesinde ne yazdığına bakılmaksızın <strong>"Seans Değil"</strong> olarak kaydedilir; seans sayınıza, cironuza veya ofis/bakıcı giderlerinize dahil edilmez.</li>
