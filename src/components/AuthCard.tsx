  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err?.code === 'auth/popup-closed-by-user') {
        setError("Giriş penceresi kapatıldı. Lütfen Google ile giriş butonuna tekrar tıklayın.");
        setLoading(false);
        return;
      }
      ...
      if (err?.code === 'auth/unauthorized-domain') {
        setError(
          "Bu web adresi (domain) Firebase yetkili alan adlarında tanımlı değil. " +
          "Lütfen yan taraftaki 'E-posta & Şifre' sekmesini kullanarak saniyeler içinde giriş yapın veya kaydolun."
        );
        setLoading(false);
        return;
      }

      // Try fallback to redirect if not in iframe
      if (!isIframe) {
        try {
      ...
      if (err?.code === 'auth/popup-blocked') {
        setError(
          "Pop-up penceresi tarayıcınız tarafından engellendi. " +
          "Lütfen pop-up pencerelerine izin verin, uygulamayı yeni sekmede açın veya 'E-posta & Şifre' sekmesini deneyin."
        );
      } else {
        setError(
          "Google Girişi tamamlanamadı. " +
          "Eğer yerleşik (iframe) bir penceredeyseniz tarayıcı pop-up engel uygulamış olabilir. " +
          "Uygulamayı yeni bir sekmede açabilir veya 'E-posta & Şifre' sekmesinden hızlıca kayıt olabilirsiniz."
        );
      }
    } finally {
      ...
      {isForgotPassword ? (
        ...
        <div className="space-y-3">
          ...
          >
            {loading ? (
              <>
                <RefreshCw className="w-4 h-4 animate-spin text-[#6b705c]" />
          ...
          {isIframe && (
            <button
              ...
              className="w-full text-center text-[11px] text-[#6b705c] hover:text-[#585c4c] font-semibold transition-colors py-1.5 flex items-center justify-center gap-1.5 cursor-pointer bg-slate-50 hover:bg-slate-100 rounded-xl border border-slate-200/60"
            >
              <ExternalLink className="w-3.5 h-3.5" />
              Pop-up engeli varsa: Uygulamayı Yeni Sekmede Açın
            </button>
          )}

          <p className="text-[11px] text-center text-slate-400 pt-1">
            Veya e-posta adresiniz ile şifre oluşturarak girmek için{' '}
            <button
              type="button"
              onClick={() => setActiveTab('email')}
              className="text-[#6b705c] font-bold underline hover:text-[#585c4c] cursor-pointer"
            >
              E-posta & Şifre
            </button>{' '}
